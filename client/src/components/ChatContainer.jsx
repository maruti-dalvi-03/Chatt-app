  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'> 
        ...
        <div className='flex items-center gap-3 py-3 mx-3 border-b border-stone-500'>
            ...
            <img src={assets.help_icon} alt="" className='w-5 md:hidden max-w-5'/>
        </div>
                ...
                <div key={index} className={`flex items-end gap-2 ${msg.senderId !== '680f50e4f10f3cd28382ecf9' ? 'justify-start' : 'justify-end'}`}>
                    {
                        msg.image ? (
                            <img src={assets.image} alt='' className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8'/>
                             ...
                             <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all ${msg.senderId !== '680f50e4f10f3cd28382ecf9' ? 'bg-gray-700 text-white' : 'bg-violet-500 text-white'}`}>{msg.text}</p>
                        )
                    }
